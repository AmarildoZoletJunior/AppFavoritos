...
from Entities.DTOs.DTOs import CategoryDTO, ExcerptDTO, FavoriteDTO, TagDTO, UserDTO
...
class Database:
    ...
    def DoSelectWithRelations(self, model, relations=None, filters=None):
        if isinstance(self.engine, str):
            return []

        with self.Session() as session:
            # Iniciar a query com o modelo completo
            ...
            # Aplicar os relacionamentos (joins) com base no mapeamento ORM
            if relations:
                for relation in relations:
                    query = query.join(relation)  # Apenas o relacionamento, sem a cláusula ON

            # Executar a consulta
            results = query.all()

            # Converter os resultados em DTOs
            dto_list = [self.convert_to_dto(result) for result in results]  # Certifique-se que a função está implementada corretamente
